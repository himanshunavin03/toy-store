...
- Wix site ID in `wix.config.json`: `b50d135b-16b5-440e-b168-47db49424421`; local `uiVersion` observed at the start of the next Run-mode iteration is `9` (the config change predated that iteration and was not edited here).
...
Home and `masterPage.js` retain the recovered `ad88861` runtime branding/hero behavior. The owner confirmed that it appears in Local Editor RUN/PLAY mode; EDIT mode still shows saved template canvas values. The current iteration changes the desktop menu to root-only to remove its visually broken children, sets the uploaded hosted hero on the candidate image element, and conditionally replaces a confirmed template category repeater. The other 15 page-code files remain empty starter callbacks. There are no application imports, backend/public calls, data queries, or custom commerce calls. Wix-generated, ignored `.wix/types/` declarations list editor elements and provide limited evidence of the site design. They are generated local metadata, not proof of configured or working live behavior.

Observed source path:

```text
Wix page or embedded widget
  -> Home.c1dmp.js / masterPage.js apply runtime branding, hero, root-only menu, and guarded Home copy/category changes
...
- Sixteen named page-code files and one `masterPage.js` exist. Home and shared code keep the recovered runtime brand/hero logic and the guarded changes described in the current handoff; the other 15 page-code files remain starter stubs.
...
**Status labels:** Catalog taxonomy, demo products, CAD prices, basic inventory, and seed idempotence are **VERIFIED** by Wix API reads. The seeder and `wix dev` startup/type sync were verified locally. The owner **CONFIRMED in RUN/PLAY mode** the recovered AnH Vibes brand, hero copy and School Bags CTA. After the current iteration the owner also confirmed the School Bags child list is gone, all eight main links including Lunch Boxes are directly visible at 1280 px, the uploaded hero image replaces the old hero, and seven text-only category links replace the three old cards. Site-specific category link clicks, product widgets, Product → Cart → Checkout, payment/shipping, member/wishlist/orders, remaining template sections, and Studio saved UI remain unverified. Do not claim a working checkout until a Wix preview walk-through confirms it.
...
| --- | --- | --- | --- | --- | --- |
| Navigation | `masterPage.js` keeps eight root links in the runtime menu and retains 36 child URLs in source only | Prior Catalog V3 read confirmed IDs/slugs | Saved native dropdown/hamburger styling remains future work | Owner confirmed no child overlay and all eight roots visible at 1280px in RUN/PLAY | Link clicks, smaller desktop widths, mobile and keyboard behavior remain checks. |
...
## Wix UI Version & Runtime Architecture

**Architecture correction (2026-09-13):** The saved Wix Studio canvas must become the AnH Vibes site. Today the owner sees Poppy & Lily and wooden-toy content in EDIT, while Local Editor RUN/PLAY executes Velo overrides that show AnH Vibes. This is a design/version gap, not a catalog problem. Keep the working overrides until each equivalent canvas change is saved and confirmed; remove them only in a later, paired code change. The current saved design's exact element values and widget bindings require inspection in the editor. The old values below are owner observations or code-matching patterns, not a canvas export.

**Responsibilities:** Wix Studio canvas owns saved text, media, menu/dropdown, card dimensions, responsive layout, static About/footer copy and native widget configuration. Velo owns genuinely dynamic behavior and API integration; it should not have to rename the template on every visit. Wix Stores Catalog V3 owns product/category records, media attached to products, prices, sale prices, options, inventory and category memberships. The Wix Dashboard is the normal editing interface for those records. The Local Editor is the design/test surface paired with local code. Git stores Velo code and the selected UI version pointer, **not the canvas snapshot**. The installed site CLI is `@wix/cli` 1.1.245; `wix dev` syncs local code and generated types/pages for the selected UI version. The installed command help exposes `dev`, `preview`, and `publish`, but no command that edits canvas layout or saves a UI version without the editor.

**Current version evidence:** At the start of this audit `wix.config.json` contains `siteId: b50d135b-16b5-440e-b168-47db49424421` and `uiVersion: "11"`. The tracked HEAD (`891ea68`) records version `9`; the local version-11 change predated this audit and was preserved. A fresh `npx wix dev` reported “Synced types for your local UI version 11,” “Synced pages for your local UI version 11,” and “Opening the Local Editor.” The generated version-11 Home/shared types still expose the hero, category repeater, menu and hamburger IDs listed below. The config is a pointer to a Wix-hosted design snapshot; it is not editable canvas JSON. Neither version number nor CLI sync proves version 11 contains the target AnH Vibes design.

**Runtime override inventory** (inspect IDs/content in Local Editor Layers before altering the canvas):

| Element ID / group | Old saved value, observed or matched | Current RUN/PLAY override | Move to saved canvas? | Runtime logic after migration? |
| --- | --- | --- | --- | --- |
| Shared `$w('Text')` in `masterPage.js` | Text containing “Poppy & Lily” | Replaces that substring with “AnH Vibes” | Yes: header/footer/site identity and original logo | No brand renaming once saved and verified |
| `#horizontalMenu1` | Template Shop All, Blocks & Stacking, Vehicles, Animals, Sale; exact saved structure needs inspection | Assigns Home plus seven root category links, with child URLs kept only in source | Yes: eight root items and eventual native accessible dropdown/hamburger | No static menu assignment once native menu works; optional future dynamic category discovery only if genuinely needed |
| Home `#text18` / `#text17` | One contains “Handcrafted Wooden Toys”; exact old supporting copy unknown | Sets “Little Things. Happy Vibes.” and “Bags, toys, school essentials and gifts for every little adventure.” | Yes, both texts | No template-text replacement |
| Home `#button6` | Saved label/link not verified | Always sets “Shop School Bags” and its existing category path | Yes: interim Shop School Bags; final Shop New Arrivals only after a real destination exists | No static label/link assignment after canvas verification |
| Home `#imageX6` | Wooden-toy hero image per owner; element identity visually confirmed in prior RUN/PLAY | Assigns uploaded Wix Media hero URL and alt | Yes: select exact Wix Media file and alt in Studio | No static image assignment |
| Home `#text19`, `#repeater1`, `#text20`, `#imageX7` | Three Blocks & Stacking / Vehicles / Animals cards; saved heading/layout requires inspection | If all three labels are present, changes heading, data to seven category IDs, card labels, URLs, uploaded image URLs and alt text | Yes: replace saved three-card design with consistent seven-card 4+3 layout and Wix category links/media | Native Wix Stores category dataset may provide live name/page URL; otherwise a narrow presentation mapping, never product fields |
| Home `$w('Text')` other template text | Text matching wooden-toy/sustainability phrases or `15%`; exact elements/copy unknown | Replaces matching copy with generic AnH copy / Back-to-School copy; collapses “Seen By” text | Yes: About, promo, section titles and footer claims | No broad template-copy sweep after saved cleanup |
| Home `#section11`, guarded by `#text38` | “Seen By” logos if heading matches | Collapses the press section when matched | Yes: remove unsupported press claim and logos in canvas | No after section removal; do not invent coverage |
| Home `#section13` / `#repeater2` cluster | Template testimonials per owner; exact content in Layers unknown | Always collapses section | Yes: remove template reviews or add real consented reviews later | No unconditional collapse once saved state is safe |

**Safe UI migration:** (1) Record the current UI version, code revision, EDIT and RUN views; do not change catalog or commerce. (2) In the Local Editor canvas, replace header brand, hero text/media/CTA and the **entire** seven-card category section together; the current repeater override stops when the three old labels are no longer all present. Keep Shop School Bags until a real New Arrivals category/gallery destination is ready. (3) Finish saved menu, remaining old copy/media, press/review removal and Wix-native galleries; check desktop/mobile. (4) Click Local Editor **Save** (and **Continue** if prompted), creating a new UI version; inspect the changed `wix.config.json`, generated page/types sync, `git diff`, and restart `wix dev`. (5) Confirm EDIT shows AnH Vibes without code, then RUN/PLAY shows the same design with real Wix category links and products. Check the regular **Edit Site** canvas separately; Wix documents that Local and Regular Editor design changes sync, while Regular Editor code comes from the default Git branch. (6) Only after those checks, remove one obsolete Velo override at a time and repeat EDIT/RUN checks. (7) A future publish must intentionally pair the reviewed code and UI version: editor publishing uses default-branch code plus latest UI, while CLI publishing uses the UI version in `wix.config.json` and the selected local/default-branch code. Push/commit/publish are **not** part of this audit. Published appearance remains unverified until a later approved release. The site CLI's shareable `wix preview` requires a previously published site; use Local Editor RUN/PLAY for pre-publication checks.

**Catalog-backed merchandising decision:** Replace old static Best Sellers/product art with native Wix Stores Grid/Slider Product Galleries using **Choose Category**. Wix states galleries can select a category; adding a product to that native category is the maintainable path for it to appear in that section. Plan separate Catalog V3 merchandising categories for New Arrivals, Popular Picks, Back to School, Trending, Best Sellers and Special Offers only when their selection rules are defined. Catalog V3 permits a product to belong to more than one category, so one product can be in School Bags, Back to School and Popular Picks without duplicate product records. These groups are **not created in this audit**, and “Trending”/“Best Sellers” should not imply metrics that have not been defined. The 13 unrelated pre-existing products mean an All Products gallery is not an acceptable AnH-only default. The current `#gridGallery1` is typed only as an iframe; its actual identity/binding must be inspected in Studio before replacing it. A Dashboard price, image, inventory or description edit should flow through native Wix product widgets without a Velo code change; a newly added product appears in a section when assigned to that gallery's chosen category.

**Direct-control limit:** This repository contains Velo files, configuration, and generated element types, not saved page/layout JSON. The site CLI has no exposed canvas-edit/UI-save command. Wix's Editor Elements API can set text and image sources only **inside a separately installed, self-managed editor add-on**, not from this site's Velo page code or current CLI; it does not cover the necessary card layout and gallery/menu configuration. No such add-on is present. Thus a small number of browser Studio actions are unavoidable: select/edit the actual canvas elements, configure native menu/datasets/galleries and responsive layout, then click Save. Exact element operations are in [WIX_STUDIO_IMPLEMENTATION.md](WIX_STUDIO_IMPLEMENTATION.md). This audit does not remove the fallback code, edit canvas, change Wix catalog data, commit, push or publish.
...
At this task's start the working tree was clean on `main` (`main...origin/main`), `wix.config.json` selected site `b50d135b-16b5-440e-b168-47db49424421` and UI version **11**, and the recovered Home and master-page Velo overrides were still present. The owner's earlier RUN/PLAY confirmation remains the latest browser evidence; EDIT is still reported as the Poppy & Lily/wooden-toy canvas. No new UI version, EDIT check, RUN check, or published check was performed in this pass.

The installed Wix CLI exposes `dev`, `preview`, and `publish`, but no command to select a Studio canvas element, change its saved design, or click Save. No Wix Local Editor browser-control tool or installed editor add-on is available here. The first unavoidable operation is selecting the visible shared-header **Poppy & Lily** text in the Local Editor canvas and saving **AnH Vibes** as its actual text. Generated shared IDs `#text2` and `#text7` are candidates only; the visible element-to-ID mapping must be checked in the Editor's Layers/Inspector before editing. Codex has stopped at this canvas gate, as requested. The owner must perform the exact first action in [WIX_STUDIO_IMPLEMENTATION.md](WIX_STUDIO_IMPLEMENTATION.md); subsequent canvas changes and Save clicks will likewise require the Editor. Do not change `uiVersion` by hand.

**Runtime safety:** `Home.c1dmp.js` and `masterPage.js` were not edited. Their brand, hero, CTA, root-menu, category-repeater, image, and old-content guards remain active. No override is safe to remove yet because no equivalent saved canvas value has been verified in both EDIT and RUN. The first Wix-native Popular Picks gallery is still unbound; future product additions must continue through **Wix Dashboard → Store Products → Add Product → images, name, description, price, inventory, real category → Save**, then a category-backed Wix Stores Product Gallery will read those records. No catalog, CMS, cart, checkout, member, order, or product data was changed. Hero and seven category assets are uploaded and Velo-connected, but their saved-canvas media connections remain pending; see [ANH_VIBES_ASSET_PLAN.md](ANH_VIBES_ASSET_PLAN.md). Next action: owner changes the saved shared-header brand in the Local Editor and clicks Save, then report the Editor's created UI version and EDIT result so synchronization and staged verification can continue.

### UI-version architecture audit (2026-09-13)

**Finding:** This local clone currently selects Wix UI version 11; `npx wix dev` synced types/pages for 11 and opened the Local Editor. HEAD records version 9, and the 9 → 11 `wix.config.json` change was already present before this audit. The owner reports EDIT still shows template content while RUN/PLAY applies AnH Vibes Velo overrides. No saved canvas migration or browser visual verification was performed here.

**Deliverables:** Added the [Wix UI Version & Runtime Architecture](#wix-ui-version--runtime-architecture) section and converted [WIX_STUDIO_IMPLEMENTATION.md](WIX_STUDIO_IMPLEMENTATION.md) into a gated migration checklist with exact Local Editor actions, code-retirement conditions, Wix-native gallery/category approach and publish-version pairing. Installed `@wix/cli` 1.1.245 help and bundled config/sync messages were inspected. Wix's official Local Editor, UI version, Git integration, gallery and Editor add-on documentation was checked. `npm run lint` and `git diff --check` passed; `wix dev` synced version 11. No Velo, category, product, media, cart, checkout or database change; no UI Save, commit, push or publish.

**Next:** In the Local Editor, complete the saved header/hero/category/menu migration gates, click Save and verify the new UI version in both regular Edit Site and RUN/PLAY. Keep the current fallback overrides until each saved equivalent works; only then retire static overrides in a separate tested change. Published mode remains a later release verification.

### Seven Wix Media category images (2026-09-13)

**Preserved starting point:** The owner had already verified AnH Vibes branding, the “Little Things. Happy Vibes.” hero and uploaded hero artwork, a clean eight-link menu including Lunch Boxes at 1280 px, no permanent child-menu overlay, and seven text-only category links in Local Editor RUN/PLAY. The saved EDIT canvas may still show wooden-toy template content. This iteration did not change `masterPage.js`, its flat menu, the hero code, Cart, Checkout, or Wix commerce records.

**Category architecture and code:** Home's generated UI types expose `#repeater1` as `$w.Repeater`; its item template contains `#text20` (`$w.Text`), `#imageX7` (`$w.Image`), and `#box46`/`#box47` (`$w.Box`); `#text19` precedes the repeater in the same element cluster. The prior owner-visible seven links proved this is the category section and that the repeater can grow beyond the three saved template items. In `Home.c1dmp.js`, the existing guard still requires all three saved labels—Blocks & Stacking, Vehicles, Animals—before replacing `#repeater1.data` with seven items and assigning “SHOP BY CATEGORY” to `#text19`. Each item now assigns its uploaded Wix-hosted image to `#imageX7.src`, its descriptive `alt`, its matching category-page link, and a linked label in `#text20.html`; the image is shown and expanded. No product/card data or extra CMS collection was created. The existing canvas repeater controls card dimensions, columns, spacing and responsive behavior; Velo does not add a new Studio layout here. Since a direct HTTPS image source is treated as a web image by Wix, Velo image crop/fit controls do not affect it; inspect 1280 px and mobile before accepting the visual layout.

**Uploaded category assets and existing Wix roots:** The owner uploaded these seven assets to Wix Media. A fresh **read-only** Catalog V3 query on the connected site returned each root category as visible, with the exact ID and slug already used by the page. The Category API supplies a slug, not a complete category-page URL, so the existing `/category/<slug>` route pattern remains a storefront link that must be checked by clicking in RUN/PLAY. Each asset URL returned HTTP 200 and `image/png` on a retrying HEAD check; this proves availability, not page rendering.

| Category | Wix category ID | Page path | Uploaded image URL |
| --- | --- | --- | --- |
| School Bags | `8fe8d0a5-f4fc-4b3b-b4a5-1da77dc2802a` | `/category/anh-vibes-school-bags` | https://static.wixstatic.com/media/339216_f561241ef8c84baa9e175aaaee7c1f29~mv2.png |
| Ladies Handbags | `d37356e5-e3ae-4791-9769-430df403c659` | `/category/anh-vibes-ladies-handbags` | https://static.wixstatic.com/media/339216_cd05ec321106425994439a78156c80c0~mv2.png |
| Soft Toys | `6513bc42-f7c0-4f0e-9474-b2733bec2a79` | `/category/anh-vibes-soft-toys` | https://static.wixstatic.com/media/339216_28e328c80cf246228773aac2974aace1~mv2.png |
| Bottles | `d07cde96-3f30-44f1-adff-d1701422688d` | `/category/anh-vibes-bottles` | https://static.wixstatic.com/media/339216_d28bdf6319874d299b39da2c4f51df6d~mv2.png |
| Stationery | `c91b348a-76b7-45e9-a7e7-19d7eecc6fc4` | `/category/anh-vibes-stationery` | https://static.wixstatic.com/media/339216_fbb962b3f74d4522a8a4979aca0f2b65~mv2.png |
| Gift Items | `a4f0d639-549f-474c-af07-f3e5698e401e` | `/category/anh-vibes-gift-items` | https://static.wixstatic.com/media/339216_5ff8f7a4c7ff4b76a745fc7ca574aa53~mv2.png |
| Lunch Boxes | `9a98a02e-664f-4d2f-b316-1d6554bfce45` | `/category/anh-vibes-lunch-boxes` | https://static.wixstatic.com/media/339216_6ea3fe30ef0344d08403cd407e66b50f~mv2.png |

**Validation and limits:** `npm run lint` passed. `npx wix dev` synced UI version 9 types/pages and reported “Opening the Local Editor.” The seven new image assignments are implemented, but the owner’s post-change RUN/PLAY visual result is pending. The tracker in [ANH_VIBES_ASSET_PLAN.md](ANH_VIBES_ASSET_PLAN.md) therefore marks all seven Uploaded and Connected YES, Verified NO. No hover/entrance animation was added before confirming the card layout. Studio canvas changes remain necessary for saved EDIT appearance, responsive card sizing/columns, native category binding if Dashboard edits should automatically rename or relink these shortcuts, and the unrelated old template sections and Wix-native product galleries. **Next:** confirm seven images/labels at 1280 px, matching click destinations, image fit and preserved menu/hero in RUN/PLAY; then address any canvas layout defects in Studio. Product data remains Wix-native; only the seven navigation labels/slugs and media references are a Velo presentation snapshot.

### Run-mode navigation and Home cleanup iteration (2026-09-13)

**Owner-confirmed starting point:** After the recovery, Local Editor RUN/PLAY visibly displayed AnH Vibes, the eight main navigation labels, “Little Things. Happy Vibes.”, its updated supporting copy and “Shop School Bags.” EDIT mode may still show original Poppy & Lily/wooden-toy canvas values because Velo `$w.onReady` overrides run only in RUN/PLAY. The owner also confirmed that School Bags children were permanently visible as a plain list over the hero, Lunch Boxes appeared under overflow at 1280 px, and the Home category/Best Sellers/promotion/About/reviews/Seen By/footer visuals still represented the old template.

**Menu fix:** `#horizontalMenu1` is typed `$w.Menu`. Its Velo API exposes `menuItems` but no supported dropdown position, panel background, padding, width, or font-size setters. `masterPage.js` now assigns only Home plus seven root category links to the visible menu. All 36 child labels/slugs/URLs remain in the `categoryNavigation` source data for a later native Studio dropdown. This is the intentionally clean MVP fallback. The menu element's canvas width, spacing, overflow behavior, and native mobile hamburger still require Local Editor inspection; code alone cannot guarantee Lunch Boxes is visible at 1280 px. Do not reinsert child `menuItems` until a styled, accessible, non-overlapping dropdown has been saved and verified in Studio.

**Home code findings:** Generated types identify `#imageX6` as an `$w.Image` beside the hero text/button cluster; `src` and `alt` are writable. The owner uploaded `hero-desktop-concept.png` to Wix Media at `https://static.wixstatic.com/media/339216_0f44dc865743436f83f3654fe6dfef9f~mv2.png`; a read-only HEAD check returned HTTP 200, image/png, 1,880,802 bytes. Home code assigns that URL and the requested alt text to `#imageX6`. **Connected/visible remains unverified** until RUN/PLAY confirms that this element is the hero rather than another image or a canvas background. The category candidate `#repeater1` is an actual `$w.Repeater`, with `#text20` text and `#imageX7` image in its generated item template. Its code replaces data with seven existing Wix category ID/label/URL shortcuts only if its rendered labels include all three old category names. Seven items can be generated from the one repeater template; no new canvas element is created. It hides the old item images because no original category images are uploaded. If the repeater is connected to a dataset or its labels do not match, this guarded change may not render; verify RUN/PLAY and use the native Wix Stores Categories app-collection binding in Studio for long-term Dashboard-driven names, links, and images.

**Old content and product section:** Home code conditionally replaces identifiable wooden-toy/sustainability and 15% text. It hides candidate `#section11` only if `#text38` confirms a “Seen By” heading. It collapses `#section13` because the generated cluster contains `#repeater2` plus star vectors, matching the owner's report of template testimonials; no fake review is substituted. Actual RUN/PLAY removal of these sections has not been separately confirmed. This does not establish that all old imagery/claims are gone. The owner reports Best Sellers shows static wooden-toy imagery; the candidate `#gridGallery1` is typed only `$w.IFrame`, so its widget identity, binding, and media are not available in source. Do not hard-code demo products there. Inspect it in Studio and replace a static gallery with a native category-backed Wix Stores Product Gallery. Popular Picks, Back-to-School and Trending must ultimately read Wix record images, names, CAD prices, stock and links. Existing Cart, Checkout and catalog data were untouched.

**Motion and assets:** No animation was added because the navigation overlap takes priority. The asset tracker in [ANH_VIBES_ASSET_PLAN.md](ANH_VIBES_ASSET_PLAN.md) records the owner's hero upload but marks it unconnected/unverified until visually seen. All other logo, category, campaign, theme, About and product media are not yet uploaded. Wix canvas work remains for menu sizing and native dropdown, responsive layout, final category art, old promotion/Best Sellers/reviews/press/footer removal, product-gallery binding and media selection.

**Validation and owner read-back:** `npm run lint`, both JS syntax checks, and `git diff --check` passed. `npx wix dev` synced UI version 9 types/pages and opened the Local Editor. Isolated `$w` code execution passed for the recovered branding/hero, eight flat menu roots, seven linked repeater items, hero source/alt and guarded old-content behavior. The owner then confirmed in actual RUN/PLAY at 1280 px: **no School Bags child overlay; all eight roots including Lunch Boxes directly visible; new uploaded hero visible; seven text-only AnH category links visible**. The old 15% promo art, static Best Sellers, About, template reviews, press logos, footer art and product gallery bindings were not separately reverified after this code change and remain canvas work until confirmed otherwise. No animation was added.

**Next step:** Keep the working runtime code. In a later Wix Studio canvas pass, replace the old saved category/Best Sellers/promo/About/reviews/press/footer content and configure native category-backed product galleries and a styled accessible submenu. Verify actual category link clicks and mobile hamburger. No commit/push, seed, product/category/price write, or cart/checkout change is part of this iteration.
...
## Category Page contextual navigation audit (2026-09-13)

**Correction to the Category Page design:** BROWSE BY should show All Products, the current root category, and only that root's children. On a leaf route, show its parent and siblings with the leaf active; retain the native breadcrumb and keep FILTER BY separate. The earlier idea of listing every root and child in one sidebar is superseded.

**Confirmed repository boundary:** `src/pages/Category Page.pdx15.js` is a starter stub. Generated `pdx15.d.ts` exposes `#categoryPage1` only as `$w.IFrame`, plus `#section4` and `#page1`; there are no page-code elements for category-menu rows, breadcrumb, filters, or product grid. The current Wix Stores Category Page is a managed widget. Wix's documented Menu settings offer all active categories or a fixed manually selected list, with no current-category/parent-context mode. Its documented Filters settings include native Price, but no Availability filter. The native category filter would put categories under FILTER BY, contrary to the requested design. Do not inject iframe messages, hard-code a static branch menu, replace the product grid, or write catalog data to work around this boundary.

**Read-only Catalog V3 evidence:** A site-scoped `POST /categories/v1/categories/query` for the existing `@wix/stores` tree returned root/child records with `id`, `slug`, and `parentCategory.id/index`. Soft Toys has Teddy Bears, Animal Plush, Character Plush, Large Soft Toys, Mini Soft Toys. School Bags has Preschool Bags, Boys Bags, Girls Bags, Character Bags, Trolley Bags, Backpack Sets; Girls Bags resolves to the School Bags parent. Bottles has Kids Bottles, Insulated Bottles, Steel Bottles, Sippers, Character Bottles. Lunch Boxes has Bento Lunch Boxes, Steel Lunch Boxes, Compartment Lunch Boxes, Character Lunch Boxes, Lunch Sets. All Products is a separate visible root. The old Blocks & Stacking, Vehicles, and Animals are still **visible Wix catalog roots**, so an automatically synchronized native menu can still show them. No catalog records were modified in this audit.

**Implementation prerequisite:** In the Wix Studio Category Page canvas, hide only the managed widget's Category menu while preserving its breadcrumb, filters, sorting, and product grid; add a separate sidebar container with BROWSE BY heading, All Products link, and a repeater/link item outside `#categoryPage1`. Save and sync so generated element IDs are available. Then page/backend code can resolve the route to a Wix Catalog V3 category, follow `parentCategory.id`, query children ordered by `parentCategory.index`, render canonical category links and active state, and leave the native widget to render products and Price. Wix's Category Page site-plugin API exposes `categoryId` to an installed site plugin, but this repository has no such plugin; ordinary page code has no direct current-category ID from `#categoryPage1`. Verify the actual site route/slug mapping and browser results before asserting the breadcrumb or dynamic price range. Availability needs a separate supported design decision because it is not among the documented native Category Page filters. No partial runtime code was added.

**Current handoff:** The contextual sidebar is **not implemented**; all requested routes and RUN/PLAY behavior remain unverified. The managed product grid and catalog are untouched. The pre-existing local `wix.config.json` UI-version change was preserved. Next action is the one-time Wix Studio canvas sidebar setup above, then implement and test data-driven page code against Soft Toys, School Bags, Bottles, Lunch Boxes, and Girls Bags. Do not commit, push, or seed.
